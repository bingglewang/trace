...
public class TraceController {
    ...
    public CommonResult<CommonPage<TraceRecordVo>> getTraceRecordByPage(String traceCodeNumber, @Valid PageParams pageParams, BindingResult bindingResult) {
        ...
        result.stream()
                .forEach(record -> {
                    Long fromSid = zslTraceSubcodeDao.selectSidByIndex(new Long(record.getTraceFromNumber() + ""), record.getTraceCodeNumber());
                    Long toSid = zslTraceSubcodeDao.selectSidByIndex(new Long(record.getTraceToNumber() + ""), record.getTraceCodeNumber());
                    record.setFromSid(fromSid);
                    record.setToSid(toSid);
                });
        return CommonResult.success(CommonPage.restPage(result));
    }
    ...
    public CommonResult pointInsert(@RequestBody TraceRecordPointParam traceRecordPointParam) {
        ...
        if (count > 0) {
            ...
            return CommonResult.failed("该批次正在发货中。。。,请稍后再试");
        }
        return CommonResult.failed();
    }
    ...
    public CommonResult buildZTPointTree(@ApiParam("摊位id") Integer stallId, @RequestParam(required = true) String traceCodeNumber, HttpServletRequest request) {
        ...
        criteria.andTraceGoodsIdEqualTo(Integer.parseInt(goodsId));
        ...
        Goods tGoods = goodsMapper.selectByPrimaryKey(Integer.parseInt(goodsId));
        if(tGoods != null){
        ...
        for (ZslTracePoint item : listz) {
            ...
            Long fromSid = zslTraceSubcodeDao.selectSidByIndex(new Long(node.getTracePointFromNumber() + ""), node.getTraceCodeNumber());
            Long toSid = zslTraceSubcodeDao.selectSidByIndex(new Long(node.getTracePointToNumber() + ""), node.getTraceCodeNumber());
            node.setFromSid(fromSid);
            node.setToSid(toSid);
            treeNodes.add(node);
        }
        try {
    ...
    public CommonResult deliverGoods(@Valid @RequestBody DeliverGoods deliverGoods, BindingResult bindingResult) {
        ZslTraceSubcode zslTraceSubcode = zslTraceSubcodeDao.selectBySubCode(deliverGoods.getCodeNumber().get(0));
        String is_deliver = redisService.get("Deliver_"+zslTraceSubcode.getTraceCodeNumber());
